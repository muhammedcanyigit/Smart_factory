...
**2026-09-18 güncellemesi**: Aktif solver Gurobi'ye geçirildi (kullanıcı akademik lisans edindi). `optimization/native_solver.py` artık Pyomo'yu sadece model kurmak için kullanıp çözümü doğrudan `gurobipy`'a devrediyor — bu, gerçekte kullanılan TEK yol (`optimization/solver.py::solve_with_warm_start` üzerinden). HiGHS devre dışı ama kod tabanından silinmedi (`optimization/solver.py::solve()`'daki basit, warm-start'sız yol hâlâ `"highs"` adını kabul ediyor, `requirements.txt`'te `highspy` yorum satırı olarak duruyor) — lisanssız bir ortama geri dönmek gerekirse tek satırlık bir geri alma. Detay: [decision-log.md](decision-log.md) "Solver Değişikliği: HiGHS -> Gurobi".

**Tarihsel not (neden başta iki ayrı arayüz vardı)**: HiGHS aktifken `optimization/solver.py` (Pyomo'nun `SolverFactory('highs')` arayüzü) ve `optimization/native_solver.py` (Pyomo'yu sadece model kurmak için kullanıp çözümü doğrudan `highspy`'a devreden yol) birlikte vardı — tesadüf değil, Phase 7-8'de bulunan gerçek bir kütüphane hatasının sonucuydu: `appsi_highs` (üçüncü bir Pyomo-HiGHS köprüsü), solver "çözüm yok" dediğinde bu ortamda sonsuza kadar takılı kalıyordu. `native_solver.py`, warm-start desteğini de koruyarak bu sorunu çözmüştü. Gurobi'nin Pyomo arayüzünde bu sınıfta bilinen bir hata olmadığı için bu ayrım artık zorunlu değil, ama mimari (Pyomo sadece model kurar, native kütüphane çözer) korunarak `gurobipy`'a taşındı — warm-start mekanizması (isim eşlemesiyle MIP start) değişmeden çalışıyor. Detay: [decision-log.md](decision-log.md) Phase 7-8.
