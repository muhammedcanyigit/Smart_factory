...
- **Öncül**: FastAPI backend + frontend. Teknoloji kararı: React yerine build-adımsız, hafif bir HTML/JS sayfası — orijinal tech stack notunda da "hızlı prototipleme gerekirse hafif framework" seçeneği vardı; npm/webpack kurulumu bu aşamada gereksiz karmaşıklık olurdu.
- **Kurulum**: `fastapi`/`uvicorn` kuruldu (requirements.txt'te zaten vardı, ortamda eksikti).
- **Uygulama**: `backend/api/routes.py` (GET /overview, /baseline, /scenarios; POST /optimize — hepsi `backend/services/pipeline.py` ve önceki fazların fonksiyonlarını çağırıyor, yeni iş mantığı yok), `backend/main.py` (FastAPI app + `frontend/` statik dosya sunumu). `frontend/index.html`: tek sayfa, vanilla JS, Plotly.js (CDN) ile before/after bar chart. Renk paleti `dataviz` skill'inin referans paletinden alındı (kategorik slot 1/2: mavi/turuncu, durum renkleri: good/warning/critical).
...
## Phase 19 — Deneysel Değerlendirme

- **Öncül**: SMALL/MEDIUM/LARGE'ın üçünde de sistematik "final" hedefiyle baseline-vs-optimized ölçümü yapılacaktı.
- **Beklenmedik, önemli bulgu — LARGE'ın model kurulumu bile 8569 saniye (2.4 saat) sürdü**: Bu, çözmeden ÖNCE, sadece Pyomo değişken/kısıt kurulumu için. MEDIUM sadece 8.53 saniyede kuruluyordu (814 operasyon) — LARGE'da operasyon sayısı 4 kat artınca kurulum süresi ~1000 kat arttı, orantısız bir büyüme.
- **Kök neden araştırması (tahmin değil, doğrulandı)**: `data_generator`'daki operasyon-tip dağılımı doğrudan sorgulandı. SMALL→LARGE arası operasyon sayısı 20 kat artarken (162→3245), aynı makine tipini paylaşan operasyon **çiftlerinin** üst sınırı (`C(n_type,2)` toplamı, yani C3 kısıtının `y[o,o']` değişken sayısı) 413 kat arttı (2.789 → 1.151.635) — `n²` büyüme deseniyle birebir uyumlu (MEDIUM/SMALL oranı 5×→25.8× ≈ 5², LARGE/MEDIUM oranı 4×→16× ≈ 4²). Gerçek ölçüm: SMALL 30.437 binary/64.990 kısıt (0.49 sn), MEDIUM 214.172 binary/856.228 kısıt (8.53 sn), LARGE **1.741.716 binary/25.465.109 kısıt (8569.40 sn)**.
- **Değerlendirme**: Bu bir kod hatası DEĞİL — Phase 5'te bilinçli olarak seçilen "ikili çakışma" (pairwise disjunctive, Big-M) kısıt formülasyonunun (C3) matematiksel olarak beklenen, literatürde bilinen bir ölçeklenme sınırı. Formülasyon SMALL/MEDIUM için doğru ve (nispeten) hızlı çalışıyor; LARGE'da operasyon-başına-tip yoğunluğu arttıkça pratik sınırına dayanıyor.
...
## Solver Değişikliği: HiGHS -> Gurobi (2026-09-18, çekirdek 24 fazın dışında, altyapı güncellemesi)

- **Öncül**: Proje boyunca (Phase 7'den beri) solver HiGHS'ti — ücretsiz/lisanssız olduğu için varsayılan seçim buydu (bkz. Phase 0 risk tablosu, Phase 7-8'deki appsi_highs donma hatası ve çözümü). Kullanıcı bu tarihte akademik bir Gurobi lisansı edindi ve projenin bundan sonra Gurobi ile çalışmasını istedi, HiGHS'in devre dışı bırakılmasını talep etti.
- **Karar**: `optimization/native_solver.py` (Pyomo'yu sadece model kurmak için kullanıp çözümü doğrudan native kütüphaneye devreden, Phase 8'de kurulmuş mimari) `highspy` yerine `gurobipy` kullanacak şekilde yeniden yazıldı. Mimari korundu — sadece alt katman değişti: MPS + SymbolMap ile isim eşleme, warm-start'ın `.Start` alanına yazılması, çözümün aynı isim eşlemesiyle Pyomo değişkenlerine geri yüklenmesi. `optimization/solver.py::solve_with_warm_start` (gerçekte kullanılan tek yol — `comparison.py` ve `predict_optimize.py` buradan çağırıyor) hiç değişmedi, çünkü zaten `native_solver.solve_native`'i solver-agnostik şekilde çağırıyordu.
- **Neden Pyomo'nun `gurobi_persistent`/`gurobi_direct` arayüzü değil de yine native (gurobipy doğrudan) yol seçildi**: Gurobi'de appsi_highs'takine benzer bilinen bir donma hatası yok — yani teorik olarak Pyomo'nun standart Gurobi arayüzlerinden biri kullanılabilirdi. Ama native yolu korumanın iki somut gerekçesi var: (1) mevcut warm-start mekanizması (SymbolMap ile isim eşleme) zaten çalışıyor ve test edilmiş durumda, Pyomo'nun kendi warm-start API'sine geçmek yeniden doğrulama gerektirirdi; (2) `Correctness > Performance` ilkesi gereği, çalışan ve anlaşılan bir mimariyi gereksiz yere değiştirmemek (`CLAUDE.md` kural 5 — gereksiz karmaşıklık ekleme).
- **Bulunan ve düzeltilen yan sorun (kod incelemesi sırasında fark edildi, bu değişiklikle birlikte giderildi)**: Eski `native_solver.py`, TÜM çağrılar arasında paylaşılan sabit bir geçici dosya yolu (`/tmp/_native_solve_model.mps`) kullanıyordu. `backend/api/routes.py`'deki `/optimize` endpoint'i senkron (`def`, `async def` değil) tanımlı olduğu için Starlette bunu bir threadpool'da çalıştırıyor — yani iki eşzamanlı istek (ör. iki tarayıcı sekmesi, ya da kullanıcının "OPTIMIZE ET"e art arda basması) aynı dosyaya aynı anda yazıp okuyabilir, biri diğerinin modelini bozup **sessizce yanlış bir sonuç** üretebilirdi. Yeni kod, `tempfile.mkstemp()` ile istek başına benzersiz bir dosya üretip işi bitince siliyor — bu Gurobi'ye özgü bir düzeltme değil, dosya zaten yeniden yazılıyorken düzeltilmiş bir genel eşzamanlılık kusuru.
- **Yan iyileştirme — feasibility tespiti**: HiGHS versiyonunda `has_feasible_solution_native`, durum string'ine ("infeasible" geçiyor mu) bakarak feasibility'yi TAHMİN ediyordu — zaman sınırına ulaşılıp hiç çözüm bulunamayan (ama "infeasible" olarak da raporlanmayan) bir durumda yanlış pozitif verme riski vardı (test edilmemiş bir köşe durumu). Gurobi versiyonunda bu, doğrudan `SolCount > 0` kontrolüne (`has_solution` alanı) dayanıyor — daha kesin.
- **Doğrulama sırası (Correctness önce, proje disipliniyle tutarlı)**: (1) `pytest tests/` — 24/24 geçti (minik el-hesabı örnekleri, Gurobi altında da Cmax=2.5 ve enerji-farkında zamanlama doğru çıktı). (2) Gerçek SMALL veri setinde `optimization/comparison.py --size small --time-limit 60`: Gurobi 60 saniyenin çok altında **kanıtlanmış optimal** çözüm buldu (gap %0, toplam maliyet iyileşmesi %12.89) — HiGHS'in Phase 8-9'da hiç ulaşamadığı bir kesinlik (o zaman en iyi sonuç gap %5-10 aralığındaydı). (3) FastAPI `TestClient` ile `/api/overview`, `/api/baseline`, `/api/optimize` uçtan uca test edildi, JSON serialization sorunsuz.
- **Kapsam dışı bırakılan (bilinçli, şimdilik)**: `docs/experiments.md`'deki Phase 9/11/15/19-20 tablolarındaki sayılar HiGHS ile ölçülmüştü — bu değişiklikle birlikte otomatik güncellenmedi (Gurobi farklı/genelde daha iyi sonuçlar buluyor, yukarıdaki %12.89 buna bir örnek). Bu tabloları Gurobi ile yeniden ölçüp güncellemek ayrı, daha kapsamlı bir iş olarak bırakıldı — istenirse yapılabilir, ama "sonuç uydurma" kuralı gereği (CLAUDE.md kural 3) şimdilik eski (HiGHS) sonuçlar olduğu gibi, tarihi kayıt olarak duruyor; karıştırılmaması için ileride bu tablolara "HiGHS ile ölçüldü" notu eklenmesi önerilir.
- **Silinmeyen, sadece devre dışı bırakılan**: HiGHS'e ait kod (`optimization/solver.py::solve()`'daki `"highs"` dalı) ve `requirements.txt`'teki `highspy` satırı (yorum satırına alındı) silinmedi — geri dönüş tek satırlık bir iş olsun diye. `config/config.yaml`'daki `solver: gurobi` alanı şu an sadece dokümantasyon amaçlı (hiçbir kod bu alanı okumuyor — gerçek seçim `native_solver.py`'ye doğrudan gömülü); bu, projede daha önce de var olan bir tutarsızlıktı (bu alan HiGHS'te de fiilen okunmuyordu), bu değişiklikle giderilmedi.

## Kod İncelemesi Sonrası İyileştirmeler (2026-09-19'dan itibaren, sırayla)

Kullanıcı, projenin tam bir kod incelemesini istedi; bulunan maddeler önem sırasına göre tek tek ele alınıyor. Bu bölüm her madde çözüldükçe büyüyecek.

### 1 — Taze `git clone` sonrası dashboard çöküyordu (ÇÖZÜLDÜ)

- **Öncül**: `ml/models/*.joblib` dosyaları reproducible oldukları için `.gitignore`'da (`git ls-files` ile doğrulandı — sadece `.gitkeep` tracked). `ml/predict_optimize.py::run_predict_optimize`, bu dosyanın var olduğunu varsayıp doğrudan `load_model(...)` çağırıyordu. Taze bir clone'da bu dosyalar hiç yok; README de sadece `pip install` + `uvicorn` diyordu, önce model eğitmek gerektiğinden hiç bahsetmiyordu. `backend/api/routes.py`'de de bunu yakalayan bir try/except yoktu — sonuç, dashboard'da yakalanmamış bir exception (HTTP 500).
- **Karar**: README'ye "önce şunu çalıştır" yazmak yeterli görülmedi, çünkü dashboard'da SMALL/MEDIUM/LARGE arasında serbestçe geçiş yapılabiliyor — hangisi seçilirse seçilsin çalışması gerekiyordu. Bunun yerine **self-healing** bir yaklaşım seçildi: `ml/prediction.py::load_or_train_model` eklendi — kayıtlı model dosyası yoksa, o boyut/görev için modeli o an eğitip diske kaydedip döner. `run_predict_optimize` artık `load_model` yerine bunu çağırıyor. Model seçim mantığı (`RMSE`'ye göre en iyisini seçme) `train_and_save_best_model` içine çıkarılıp hem CLI (`__main__`) hem yeni fonksiyon arasında paylaşıldı (kod tekrarı yok).
- **Doğrulama**: `ml/models/processing_time_small.joblib` geçici olarak kaldırılıp `run_predict_optimize(size="small", ...)` çalıştırıldı — model kendini eğitip aynı doğru sonucu (`total_cost=11995.72`, bir önceki (madde bulunmadan önceki) ölçümle birebir aynı) üretti, dosya diske tekrar yazıldı. `pytest tests/` (24/24) tekrar geçti.
- **Kapsam dışı bırakılan**: Backend'e (routes.py) genel bir try/except eklemek bu maddenin kapsamında değildi — ayrı bir madde (aşağıdaki listede "API hata yönetimi eksik") olarak bekliyor.

### 2 — `requirements.txt` sürüm pin'siz (ÇÖZÜLDÜ)

- **Öncül**: Dosya Phase 0'dan beri "taslak, ilgili faza gelince sabitlenecek" notuyla hiçbir sürüm numarası içermiyordu (`git log` ile doğrulandı — dosya Phase 0'dan bu yana hiç değişmemiş). Bu, projenin kendi öncelik sıralamasındaki 3. madde (Reproducibility) ile çelişiyordu — appsi_highs'ın Phase 7-8'de bulunan donma hatası tam olarak bir sürüm kombinasyonuna (Pyomo 6.10.1 + belirli bir highspy sürümü) bağlıydı.
- **Karar**: Kurulu/çalışan ortamdaki gerçek sürümler tek tek tespit edilip (`importlib.metadata`) dosyaya yazıldı — rastgele/tahmini bir "en güncel sürüm" değil, gerçekten test edilmiş sürümler. Eksik bir doğrudan bağımlılık da fark edildi ve eklendi: `joblib` (`ml/prediction.py`'de doğrudan import ediliyor ama dosyada hiç yoktu, örtük olarak scikit-learn üzerinden geliyordu).
- **Doğrulama**: Bu değişiklik yalnızca bir metin dosyasını (kurulu paketleri değil) etkilediği için ayrı bir test gerekmedi — sürümler zaten o an pytest/comparison/API testlerinin geçtiği ortamdan alındı.

### 3 — Ölü konfigürasyon: `.env.example`, `database:`/`api:` blokları (ÇÖZÜLDÜ)

- **Öncül**: `grep -rn "os.environ|dotenv|getenv"` ve `grep -rn '\["database"\]|\["api"\]'` sıfır sonuç verdi — bunlar Phase 0'ın orijinal mimari planından (SQLite ile kalıcı depolama, env-var tabanlı config) kalma, ama proje SQLite'ı hiç kullanmadan tamamen in-memory/stateless (her istek `generate_dataset()`'i sıfırdan çağırıyor) ilerledi. Phase 22 "mimari sadeleştirme" bu tür ölü kodu temizlemek için yapılmıştı (`experiments/optimization`, `experiments/ml` kaldırılmıştı) ama bu ikisi gözden kaçmıştı.
- **Karar**: `.env.example` silindi. `config.yaml`'daki `database:`/`api:` blokları kaldırıldı. `optimization.solver` alanına (kaldırılmadı, çünkü en azından dokümantasyon değeri var) bu alanın kod tarafından okunmadığını açıkça belirten bir not eklendi.
- **Doğrulama**: `pytest tests/` (24/24), `backend.main` import + `yaml.safe_load` smoke testi.

### 4 — Dashboard, bilinen ölçeklenme sınırını yansıtmıyordu (ÇÖZÜLDÜ)

- **Öncül**: `frontend/index.html`'de SMALL/MEDIUM/LARGE eşit sunuluyordu. LARGE seçilip OPTIMIZE ET'e basılırsa Phase 19'a göre sadece model kurulumu 2,4 saat sürebiliyor — ve "Solver Süresi" alanı (max 300sn) bunu sınırlamıyor çünkü o parametre sadece `solve()` adımını kapsıyor, `build_model`'i değil. MEDIUM da "final" hedefte (Gurobi öncesi) yüksek gap veriyordu, bu da UI'da hiç belirtilmiyordu.
- **Karar**: Boyut seçildiğinde beliren, uçtan uca (backend'e dokunmadan, sadece frontend'de) bir uyarı kutusu eklendi — MEDIUM için "gap yüksek çıkabilir" (sarı), LARGE için "model kurulumu dakikalar-saatler sürebilir, Solver Süresi bunu sınırlamaz" (kırmızı, daha vurgulu). Backend/pipeline'a dokunulmadı — bu bilinçli bir "engelleme değil, bilgilendirme" kararı: kullanıcı LARGE'ı yine de deneyebilir (ör. sabırlıysa ya da farklı bir makinede), sadece bilmeden saatlerce beklemesin diye uyarılıyor.
- **Doğrulama**: Playwright (headless Chromium) ile boyut seçici değiştirilip uyarı metninin doğru göründüğü/SMALL'da kaybolduğu ve konsol hatası olmadığı doğrulandı.

### 5 — `replay_with_ground_truth` docstring'i eski/yanlıştı (ÇÖZÜLDÜ)

- **Öncül**: `ml/predict_optimize.py::replay_with_ground_truth`'un docstring'i "bakım pencerelerini yeniden kontrol etmiyor" diyordu. Ama fonksiyon `replay_schedule`'ı `maintenance_by_machine=build_maintenance_lookup(...)` ile çağırıyor ve bu, bakım çakışmasını aktif olarak kontrol edip başlangıcı bakım bitimine itiyor — tam olarak Phase 11'de düzeltilen sorun. Docstring, muhtemelen genel `replay_schedule` fonksiyonuna taşınmadan önceki bir versiyondan kalmıştı; kod doğruydu, yalnızca yorum yanlıştı.
- **Karar**: Docstring, fonksiyonun gerçekte ne yaptığını (bakım kontrolü dahil, `apply_warm_start` ile paylaşılan ortak mantık) anlatacak şekilde düzeltildi. Kod değişmedi.
- **Doğrulama**: `pytest tests/` (24/24) — davranış zaten doğruydu, sadece dokümantasyon güncellendi.

### 6 — Backend/API hiç test edilmiyordu (ÇÖZÜLDÜ)

- **Öncül**: 24 pytest testinin hiçbiri `backend/`'i kapsamıyordu — Phase 17'deki gerçek bir regresyon (ham `HighsModelStatus` enum'unun JSON response'a sızması) elle/Playwright ile bulunmuştu, pytest ile değil. Aynı sınıf bir hata (ör. Gurobi geçişinde `model_status`'ın artık int olması ama gelecekte başka bir alanın yine serialize edilememesi) tekrar olursa otomatik yakalanmazdı.
- **Karar**: `tests/test_backend.py` eklendi — FastAPI `TestClient` ile `/api/overview`, `/api/baseline`, `/api/scenarios`, `/api/optimize` (gerçek SMALL veri setiyle) ve geçersiz senaryo için `400` kontrolü. `test_optimize_response_is_json_serializable_and_feasible`, Phase 17'nin doğrudan regresyon testi — `solve_info`'da `model_status` anahtarının bulunmadığını assert ediyor.
- **Bilinçli ödünleşim**: Phase 21'in "MILP testleri saniyeler içinde bitmeli" ilkesinin (tiny_dataset fixture) aksine, bu dosya GERÇEK SMALL veri setiyle gerçek bir MILP çözüyor (~11sn, `time_limit_seconds=10`) — çünkü amaç burada "model doğru mu" değil "API/serialization uçtan uca doğru mu" sorusu, ve bunu gerçekçi test etmenin yolu gerçek pipeline'ı çalıştırmaktan geçiyor. `README.md`'deki test süresi iddiası (`21+ test, ~1.2sn`) artık doğru olmayacağı için gerçek ölçülen süreyle (`29 test, ~12sn`) güncellendi — CLAUDE.md kural 3 (sonuç uydurmama) gereği.
- **Doğrulama**: `pytest tests/` — 29/29 geçti (~11,5sn).

### 7 — API hata yönetimi eksikti (ÇÖZÜLDÜ)

- **Öncül**: `routes.py`'de hiç try/except yoktu. `size` parametresi hiç doğrulanmıyordu — geçersiz bir değer `config["dataset"]["presets"][size]`'da `KeyError`'a yol açıp çıplak bir `500` (stack trace) döndürüyordu. Frontend'in `catch` bloğu da hata gövdesini (`result.feasible`/`result.metrics` gibi olmayan alanlara erişip) düzgün okumuyordu — kullanıcı anlamsız bir mesaj görüyordu.
- **Karar**: Backend'de `_validate_size()` (400) ve `/optimize`'daki pipeline çağrısının try/except ile sarmalanması (500, okunabilir `detail` mesajı) eklendi. Frontend'de `readJsonOrThrow()` eklendi — `res.ok` kontrolü yapıp `detail`'i okuyarak düzgün bir `Error` fırlatıyor.
- **Doğrulama**: `pytest tests/` (31/31, 2 yeni test: geçersiz `size` için `/optimize` ve `/overview`'ın `400` döndürdüğü). Playwright ile happy-path (OPTIMIZE ET → Before/After) yeniden test edildi — paylaşılan fetch/parse mantığı değiştiği için regresyon riski vardı, konsol hatası yok, sonuç (%8,09 iyileşme, optimal) doğru çıktı.

### 8 — `docs/experiments.md` güncel değildi (ÇÖZÜLDÜ)

- **Öncül**: Solver değişikliği (Gurobi) sonrası Phase 9/11/15/19'daki sayılar hâlâ HiGHS döneminden kalmıştı — kullanıcıya sohbette bu fark (özellikle MEDIUM: %0 → %11,81) doğrudan raporlanmıştı ama dokümana işlenmemişti.
- **Karar**: Eski tablolar SİLİNMEDİ — CLAUDE.md kural 3 (sonuç uydurma/silme yok) ve "ne değişti" değil "ne oldu"yu koruma ilkesiyle, o zaman gerçekten ölçülmüş, doğru sayılardı. Bunun yerine: (1) ilgili başlıklara "HiGHS ile ölçüldü, tarihi kayıt" notu eklendi, (2) dosyanın sonuna yeni bir "Solver Güncellemesi: Gurobi" bölümü eklendi — SMALL/MEDIUM/Predict→Optimize için aynı ayarlarla yeniden ölçülmüş Gurobi sayıları, HiGHS ile yan yana tablolandı. LARGE bilinçli olarak yeniden ölçülmedi (solver'dan bağımsız bir darboğaz, 2,4 saati boşa harcamanın anlamı yok) — bu gerekçe de dokümana açıkça yazıldı.
- **Doğrulama**: Sadece dokümantasyon değişikliği; `pytest tests/` (31/31) yine de tekrar çalıştırıldı, etkilenmedi.
